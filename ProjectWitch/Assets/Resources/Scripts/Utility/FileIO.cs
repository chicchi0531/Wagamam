...
namespace ProjectWitch
{
    ...
    public abstract class ISaveableData
    {
        ...
        public abstract int SetFromBytes(int offset, byte[] data, int Major, int Minor);
    ...
    static class FileIO
    {
            ...
            where T2 : ISaveableData 
        {
            using (FileStream fs = new FileStream(filepath, FileMode.Open))
            {
                ...
                if(metaData.Minor != meta.Minor)
                {
                    Debug.LogWarning("セーブデータを更新します。");
                }

                //データの読み出し
                buffer = new byte[fs.Length-meta.GetSize()];
                fs.Read(buffer, 0, buffer.Length);
                data.SetFromBytes(0, buffer, metaData.Major, metaData.Minor);
