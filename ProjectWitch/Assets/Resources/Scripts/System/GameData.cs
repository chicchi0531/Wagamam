...
namespace ProjectWitch
{
	...
	public class GameData : ISaveableData
	{
        ...
        //時間帯を同期するメモリのID
        [SerializeField]
        private int mCurrentTimeID = 4013;
        //現在の時間数 0:朝 1:昼 2:夜 3~:敵ターン
        private int mCurrentTime = 0;
		public int CurrentTime { get { return mCurrentTime; } set { mCurrentTime = value; Memory[mCurrentTimeID] = value; } }

        //エリ改革フラグを同期するメモリのID
        [SerializeField]
        private int mErisReformID = 4016;
        public bool IsAfter_ErisReform { get { return Memory[mErisReformID] == 0 ? false : true; } }

        //占領地域数を同期するメモリのID
        [SerializeField]
        private int mDominatedAreaCountID = 4017;
        public int DominatedAreaCountID { get { return mDominatedAreaCountID; } }
        ...
        //自領地のユニットリストをソートする
        public List<int> SortUnitList(List<int> unitList, EUnitListSortType type)
        {
            var game = Game.GetInstance();

            switch(type)
            {
                case EUnitListSortType.ID:                   return unitList.OrderBy(x => x).ToList();
                case EUnitListSortType.Job:                  return unitList.OrderBy(x => (int)(game.GameData.Unit[x].Job)).ToList();
                case EUnitListSortType.Level_Ascending:      return unitList.OrderBy(x => game.GameData.Unit[x].Level).ToList();
                case EUnitListSortType.Level_Descending:     return unitList.OrderByDescending(x => game.GameData.Unit[x].Level).ToList();
                case EUnitListSortType.SolNum_Ascending:     return unitList.OrderBy(x => game.GameData.Unit[x].MaxSoldierNum).ToList();
                case EUnitListSortType.SolNum_Descending:    return unitList.OrderByDescending(x => game.GameData.Unit[x].MaxSoldierNum).ToList();
            }
		...
		{
            //ゲームクラスのシーン間連携データをリセット
            var game = Game.GetInstance();
            game.ResetSceneInputData();
		...
		public void Copy(GameData inst)
        {
            ...
            mCurrentTurnID = inst.mCurrentTurnID;
			CurrentTurn = inst.CurrentTurn;

            TownEventEnable = inst.TownEventEnable;

			FieldBGM = inst.FieldBGM;
			BattleBGM = inst.BattleBGM;
            TownBGM = inst.TownBGM;
            ShopBGM = inst.ShopBGM;
		}
		...
		public override byte[] GetSaveBytes()
		{
            ...
            outdata.AddRange(BitConverter.GetBytes(TownEventEnable));
		...
		//byte配列からデータを再現
		public override int SetFromBytes(int _offset, byte[] data,int Major,int Minor)
		{
			int offset = _offset;
			...
			{
				offset = Unit[i].SetFromBytes(offset, data, Major, Minor);
			}
			...
			{
				offset = Area[i].SetFromBytes(offset, data, Major, Minor);
			}
			...
			{
				offset = Territory[i].SetFromBytes(offset, data, Major, Minor);
			}
			...
			{
				offset = Group[i].SetFromBytes(offset, data, Major, Minor);
			}
            ...
            TownEventEnable = BitConverter.ToBoolean(data, offset); offset += 1;
			...
			offset = Memory.SetFromBytes(offset, data, Major, Minor);
	...
	public class SystemData : ISaveableData
	{
		...
		public override int SetFromBytes(int _offset, byte[] data,int Major, int Minor)
		{
			...
			//データ代入
			offset = Config.SetFromBytes(offset, data, Major,Minor);
			offset = Memory.SetFromBytes(offset, data,Major,Minor);
    ...
    //並び替え用
    public enum EUnitListSortType
    {
        ID,                 //ID順
        Level_Ascending,    //レベル昇順
        Level_Descending,   //レベル降順
        Job,     //種族昇順
        SolNum_Ascending,   //兵数昇順
        SolNum_Descending   //兵数降順
    }
	...
	public class UnitDataFormat : ISaveableData
	{
        ...
        //HP
        private int mHP = 0;
		public int HP { get { return Math.Min(MaxHP, mHP); } set{ mHP = value; } }
		//最大HP
            ...
            get
            {
                ...
                else
                {
                    ...
                    if (game.GameData.IsAfter_ErisReform)
                        return 3.0f + 2.0f * ((MaxSoldierNum / 100.0f) - 10.0f);
                    else
		...
		//装備を含めたステータス
		public int MaxHP { get { return MathEx.GTO(BaseMaxHP + EquipmentData.MaxHP); } private set { } }
		public int LeaderPAtk { get { return MathEx.GTZ(BaseLPAtk + EquipmentData.LeaderPAtk); } private set { } } //物理攻撃
		public int LeaderMAtk { get { return MathEx.GTZ(BaseLMAtk + EquipmentData.LeaderMAtk); } private set { } } //魔法攻撃
		public int LeaderPDef { get { return MathEx.GTZ(BaseLPDef + EquipmentData.LeaderPDef); } private set { } } //物理防御
		public int LeaderMDef { get { return MathEx.GTZ(BaseLMDef + EquipmentData.LeaderMDef); } private set { } } //魔法防御
		public int GroupPAtk { get { return MathEx.GTZ(BaseGPAtk + EquipmentData.GroupPAtk); } private set { } }  //物理攻撃
		public int GroupMAtk { get { return MathEx.GTZ(BaseGMAtk + EquipmentData.GroupMAtk); } private set { } }  //魔法攻撃
		public int GroupPDef { get { return MathEx.GTZ(BaseGPDef + EquipmentData.GroupPDef); } private set { } }  //物理防御
		public int GroupMDef { get { return MathEx.GTZ(BaseGMDef + EquipmentData.GroupMDef); } private set { } }  //魔法防御
		public int Leadership { get { return MathEx.GTZ(BaseLeader + EquipmentData.Leadership); } private set { } }  //指揮力
		public int Curative { get { return MathEx.GTZ(BaseCur + EquipmentData.Curative); } private set { } }       //回復力
		public int Agility { get { return MathEx.GTZ(BaseAgi + EquipmentData.Agility); } private set { } }        //機動力
	...
	public class AreaDataFormat : ISaveableData
	{
        ...
        public static bool AreaManaIsLarge(AreaDataFormat area)
        {
            var game = Game.GetInstance();
            var turn = game.GameData.CurrentTurn;

            var threthold = 200 * turn / 2 + 1500;

            return threthold <= area.Mana;
        }
	...
	public class ConfigDataFormat : ISaveableData
	{
		public ConfigDataFormat()
		{
            ...
            UnitListSortType = EUnitListSortType.ID;
		}
        ...
        //ユニットのソート順
        public EUnitListSortType UnitListSortType { get; set; }
		...
		public override byte[] GetSaveBytes()
		{
            ...
            outdata.AddRange(BitConverter.GetBytes(VoiceVolume));
            outdata.AddRange(BitConverter.GetBytes((int)UnitListSortType));
		...
		public override int SetFromBytes(int _offset, byte[] data, int Major, int Minor)
		{
            ...
            if(Major==0 && Minor==1)
            {
                VoiceVolume = BitConverter.ToSingle(data, offset); offset += 4;
                UnitListSortType = EnumConverter.ToEnum<EUnitListSortType>(
                    BitConverter.ToInt32(data, offset)); offset += 4;
            }
	...
	public class VirtualMemory : ISaveableData
	{
		...
		{
			get { Update(index);  return Data[index]; }
        ...
        private void Update(int index)
        {
            ...
            //占領地域数の取得
            if (index == gamedata.DominatedAreaCountID)
            {
                Data[index] = gamedata.Territory[0].AreaList.Count;
            }
        }
		...
		static string[] opeStr =
{
	...
	public class BattleDataIn
	{
		...
		void Init()
		{
            ...
            PlayerStopType = GroupDataFormat.BattleType.ToDestroyedAll;
            EnemyStopType = GroupDataFormat.BattleType.ToDestroyedAll;
        ...
        //バトルの中断条件（プレイヤーはオートバトルのとき以外は0）
        public GroupDataFormat.BattleType PlayerStopType { get; set; }
        public GroupDataFormat.BattleType EnemyStopType { get; set; }
    ...
    public class TownDataIn
    {
        ...
        public int AreaID { get; set; }
        public Field.FieldController FController { get; set; }
        public bool FromTown { get; set; } //町シーンから遷移したか、フィールドから遷移したか
    }
